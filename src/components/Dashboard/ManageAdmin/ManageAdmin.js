...
const AddAdmin = () => {
  ...
  useEffect(() => {
    ...
    fetch("https://memory-makers-photography-server.vercel.app/admins")
      .then((res) => res.json())
  ...
  const onSubmit = (data) => {
    toast.promise(
      fetch("https://memory-makers-photography-server.vercel.app/addAdmin", {
        method: "POST",
  ...
  const handleRemove = (_id) => {
    swal({
      ...
      if (willDelete) {
        fetch(
          `https://memory-makers-photography-server.vercel.app/deleteAdmin/${_id}`,
          {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
          }
        ...
        <form className="row g-3 mt-2" onSubmit={handleSubmit(onSubmit)}>
          <div className="mx-auto form-modal">
            ...
            <div className="col mt-5">
              <input
                className="d-block btn btn-info ms-auto"
                type="submit"
                value="Add Admin"
              />
            </div>
                ...
                <tr key={_id}>
                  ...
                  <td className="text-center">
                    <FontAwesomeIcon
                      ...
                      icon={faTrashAlt}
                      className="text-danger fs-3 action-icon"
                    />
                  </td>
                </tr>
              ))}
