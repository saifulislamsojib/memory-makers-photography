...
const Profile = () => {
  ...
  const updateUser = (data) => {
    toast.promise(
      updateProfile(data.name, imageUrl).then((isUpdated) => {
        if (isUpdated) {
          setLoggedInUser((preUser) => ({
            ...preUser,
            name: data.name,
    ...
    <>
        ...
        <div className="text-center p-4 radius profile">
          <div className="upload-image">
            <input
              ...
              className="d-none"
            />
            <label className="upload-btn btn" htmlFor="user-img">
              Upload
            </label>
            <img
              ...
              src={
                imageUrl ||
                photo ||
                "https://uxwing.com/wp-content/themes/uxwing/download/12-people-gesture/avatar.png"
              }
              alt=""
            />
          </div>
          <form onSubmit={handleSubmit(updateUser)}>
            ...
            <button
              disabled={uploading}
              className="btn btn-outline-primary d-block ms-auto profile-uploading-btn"
            >
              Update
            </button>
          </form>
      ...
      <div className="d-flex align-items-center justify-content-center profile-container">
        <div className="text-center shadow p-4 radius profile">
          ...
          >
            Edit <FontAwesomeIcon icon={faEdit} className="ms-2" />
          </button>
        </div>
      </div>
    </>
