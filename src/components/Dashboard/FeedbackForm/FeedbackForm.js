...
const FeedbackForm = ({ feedbackData, setFeedbackData, setIsOpen }) => {
  ...
  const onSubmit = (data) => {
    if (ratings) {
      ...
      const path = feedbackData
        ? `updateFeedback/${feedbackData._id}`
        : "sendFeedback";
      const allData = feedbackData
        ? { ...data, ratings, photo }
        : {
            ...data,
            email,
            ...
            if (inserted) {
              if (feedbackData) {
                ...
                swal(
                  ...
                  "Your feedback updated successfully done!",
                  "success"
                );
                setIsOpen(false);
              } else {
                ...
                swal(
                  ...
                  "Your feedback submitted successfully done!",
                  "success"
                );
              }
            } else {
              ...
              swal(
                ...
                "Your feedback not submitted successfully!",
                "error"
              );
            }
          }),
  ...
  return (
    ...
    >
      ...
      <div className="col-12">
        <input
          defaultValue={feedbackData?.company}
          type="text"
          className="form-control"
      ...
      <div className="col-12">
        <textarea
          defaultValue={feedbackData?.description}
          style={{ height: "200px" }}
          className="form-control"
        ...
        {errors.description && (
          <span className="text-danger d-inline-block mt-2">
            description is required minimum 10 character
          </span>
        )}
      </div>
        ...
        {["rating1", "rating2", "rating3", "rating4", "rating5"].map(
          (rating, i) => (
            <FontAwesomeIcon
              key={rating}
              className={`${rating} ${
                i < hoverRatings ? "selected-rating" : ""
              } ${i < ratings && hoverRatings === 0 ? "selected-rating" : ""}`}
              icon={faStar}
              onClick={() => setRatings(i + 1)}
              onMouseEnter={() => setHoverRatings(i + 1)}
