...
const Login = () => {
  ...
  const { loggedInUser, setLoggedInUser, loading, setAdminLoaded, isDarkMode } =
    useContext(context);
  ...
  useEffect(() => {
    document.title = "Login - Memory Makers";
  }, []);

  const sendMail = () => {
    toast.promise(
      sendEmailVerification(from.pathname).then((res) => {
        if (res) {
  ...
  const onSubmit = (data) => {
    ...
    if (newUser) {
      ...
      const toastId = toast.loading("Loading, Please Wait...");
      signingUser(email, password).then((res) => updateState(res, toastId));
    }
  };

  const SignInWithProvider = (provider) => {
    const toastId = toast.loading("Loading, Please Wait...");
    provider().then((res) => updateState(res, toastId));
  };
    ...
    if (!loading && !loggedInUser.email) {
      toast.custom(
        ...
        >
          <div>
            {" "}
            <div className="text-center">For Admin</div> Admin Email:
            saifulsojibdev@gmail.com
          </div>
          <div>Admin Password: 144395</div>
        </div>
      );
    }
    ...
    <>
      {!loading && (
        <div className="my-2 position-absolute exit-container">
          <button onClick={() => history.push("/")} className="btn primary-btn">
            Go TO Home
          </button>
        </div>
      )}
              ...
              {emailVerification || loggedInUser.emailVerified === false ? (
                <div className="alert-warning text-center p-4 radius">
                  ...
                  <small className="d-block">
                    you received an email at: {loggedInUser.email}
                  </small>
                  <button
                    onClick={sendMail}
                  ...
                  <button
                    ...
                    className="mt-2 btn btn-outline-danger d-block mx-auto"
                  >
                    Log Out
                  </button>
                </div>
                ...
                <div className="my-4 mx-auto mx-lg-0 login-inner">
                  <form
                    className="email-password-login"
                  ...
                  >
                    ...
                    <div className={newUser ? "form-floating my-3" : "d-none"}>
                      ...
                      {errors.name && (
                        <span className="text-danger d-inline-block mt-2 ms-2">
                          Name is required
                        </span>
                      )}
                    </div>
                    ...
                    <div
                      className={
                        newUser
                          ? "form-floating my-3 password-section"
                          : "d-none"
                      }
                    >
                      <input
                        ...
                        className="input form-control"
                        type={showPassword ? "text" : "password"}
                        {...register("confirmPassword", { required: newUser })}
                        id="ConfirmPassword"
                    ...
                    <input
                      className="input w-100 btn btn-outline-success my-3 p-2"
                      type="submit"
                    ...
                    <h6 className="text-center">
                      ...
                      <span
                        ...
                        className="text-primary ms-2 create-account"
                      >
                  ...
                  <div className="d-flex justify-content-center header-icon">
                    <div className="header-links link-1 mx-1">
                      <FontAwesomeIcon
                        onClick={() => SignInWithProvider(fbSignIn)}
                        icon={faFacebook}
                      />
                    </div>
                    <div className="header-links link-3 mx-1">
                      <FontAwesomeIcon
                        onClick={() => SignInWithProvider(googleSignIn)}
                        icon={faGoogle}
                      />
                    </div>
                  </div>
